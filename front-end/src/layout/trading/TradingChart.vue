...
import { getOHLCV, listTrades } from "@/plugins/backend";
...
export default {
  ...
  methods: {
    ...
    listTrades() {
      ...
      getOHLCV(this.pairId, fromTimestamp, toTimestamp, this.intervalType).then(res => {
    ...
    rangeChange: debounce(function (data) {
      if (this.isChartReset) {
        this.isChartReset = false;
        return;
      }
      const fromTimestamp = Math.floor(data[0] / (60 * 1000)) * 60 * 1000 + 60 * 1000;
      const toTimestamp = this.data.data.chart.data[0][0] - 1;
